...
namespace VRbJS {
	namespace JSUtils {
		...
		public class Binder {
			...
			public virtual void bind(string name, bound_function cb, bool constructor = false, int n_args = -1, ValueType?[] atypes = null, string[]? anames = null) {
				if (constructor) {
		...
		;
			...
			public string compile_bridge_code(JSCore.Context c, Binder? klass = null, out JSCore.Value e) {
				...
				return (string)((JSUtils.Context)c).exec("""Opal.compile("%s");""".printf(code.escape(null)));
			}	
			...
			public string generate_bridge_code(JSCore.Context c, owned Binder? klass = null, out JSCore.Value e) {
				...
				Gee.HashMap<string, BoundFunction>? map = ((Gee.HashMap<string, BoundFunction>?)Type.from_instance(this).get_qdata(Quark.from_string("map")));
				...
				map = ((Gee.HashMap<string, BoundFunction>?)Type.from_instance(this.target).get_qdata(Quark.from_string("map")));
				...
				foreach (var val in map.entries) {
					...
					code += @"\n  def $(val.key) *o,&b\n    o.push(b) if b;\n    $((val.key in this.target.constructors) ? "self.class.wrap " : "")`#{@_native}['$(val.key)'].apply(#{@_native}, #{o})` \n  end\n";
